...
- Use `./scripts/aws-cost-guard-deploy.sh` to install or dry-run the USD 10 AWS actual/forecast cost guard. Default scope is resources named/tagged with `kbo-fans`; use account-wide or destructive mode only when the AWS account is dedicated to this project or fixed-cost resource deletion is explicitly intended.
