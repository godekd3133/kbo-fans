...
- 처음 env 파일을 만들 때는 `./scripts/push-demo-env-bootstrap.sh --output /tmp/kbo-fans-aws.env --force`로 로컬 Firebase client config 경로와 project id가 반영된 초안을 만들 수 있다. Apple/APNs/AWS placeholder는 그대로 audit에서 잡히게 둔다.
