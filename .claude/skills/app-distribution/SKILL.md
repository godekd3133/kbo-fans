...
- For tester-facing iOS release work, do not stop at TestFlight upload. After App Store Connect processing reaches `VALID`, attach the newest build to the `External Testers` group, remove superseded older build relationships from that group, and submit Beta App Review when no submission exists yet. Keep upload success, Apple processing/VALID, external-group assignment, Beta App Review, and actual installability as separate checkpoints.
