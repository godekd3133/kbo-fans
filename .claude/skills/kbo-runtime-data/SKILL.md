...
- Current-season team players / team stats / player detail must be fresh-first and fail-visible in explicit API-backed mode. Do not mask failures with backend/app/device snapshots.
...
- Current/live game-detail relay, boxscore, and lineup may keep the last identity- and displayability-validated response in a bounded 60-second local cache while remaining network-first. Use it only for connection/timeout/gateway failures, mark the model as stale in the UI, and bypass it for manual refresh; do not persist empty, partial, cross-game, or unavailable payloads.
