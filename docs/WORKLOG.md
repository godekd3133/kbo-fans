...
- 변경(`scoreboard.py`):
  ...
  - `get_game`/`get_scoreboard`의 UpstreamBusy 폴백 경로에도 같은 마커 게이트 적용. `home`/`compact`/`prime_home_scoreboard`도 마킹된 당일 날짜의 `scoreboard` 스냅샷을 재사용 — 전 경기 종료 후 저녁 시간대의 홈/위젯 재크롤 제거.
...
### 결정
- 이전 세 페르소나를 최초 설치, 무팀, 재방문 마이팀, 빠른 점수, 문자중계 장기 독자, 연장 경기, 기록 신뢰, 경기 전 라인업, 느린 네트워크, 일정·구장, 순위, 기록실, 선수 상세, 출처 민감, 알림함, 권한 거부, 320px, 큰 글자·보조기술, 업데이트 복귀 등 20개 맥락으로 확장한다.
- 화면이 그럴듯한지보다 실제 데이터 계약과 표현이 맞는지를 우선한다. 원천 필드가 없는 LIVE B/S/O·루상, 비공식 `결승타`, 고정 9회 표, 생성형 데이터의 `뉴스` 표기는 P1으로 다룬다.
- `/news` route와 내부 class 이름은 호환을 위해 유지하되 사용자 화면명은 `데이터 브리핑`, 하단 탭은 `브리핑`으로 바꾼다. 외부 기사로 오인하지 않게 자동 정리·생성 시각·계산형 여부를 표시한다.
- 앱 동작·데이터 표현·화면명·업데이트 소식이 함께 바뀌므로 `0.1.20` 재업로드가 아니라 `0.1.21+89` / tag `0.1.21` 새 tester-facing release로 올린다.

### 진행
- [x] 홈 LIVE 카드의 고정 B/S/O `3/2/2`와 고정 루상을 제거하고 원천 상황 정보 미제공을 명시.
- [x] 스코어 탭을 최소 9회 이상의 동적 이닝 표, 고정 팀 열, 가로 스크롤, R/H/E/B 범례와 접근성 요약으로 교체.
- [x] 박스스코어의 `결승타/생산 +N`을 `오늘의 활약 타자/앱 기준 활약 지수`로 바꾸고 원시 예외 대신 안전한 오류·재시도를 제공.
- [x] 투수 파생값도 `앱 기준 투구 효율`로 고지하고 320px 활약 행은 태그·선수명·계산 지표를 세로 배치해 이름 소거를 방지.
- [x] 값 없는 선발 비교표를 숨겨 공개 타순을 먼저 보여주도록 라인업 정보 우선순위 조정.
- [x] `/home.meta.generatedAt`을 `HomeAggregate.generatedAt`으로 보존하고 데이터 브리핑에 자동 정리·실제 기사 아님·생성 시각 또는 미제공 상태를 표시.
- [x] 마이팀 관련 흐름을 브리핑 리드에서 먼저 배치하고, 시즌 환산은 `앱 계산`으로 표시하며 실제 2경기 차 이내에서만 `턱밑` 사용. 편집 제목 접두사 중복 방지.
- [x] 기록실 첫 요약의 반복 headline·수치 상자·설명 문장을 줄이고 타자·투수 대표 다음에 지표 rail과 TOP 5가 바로 이어지게 정리.
- [x] 선수 상세의 `헤드라인/현재 상태/시즌` 반복을 지표명·값 카드로 바꾸고 빈 값·주요/시즌 동일 지표 반복 숨김, 320px·큰 글자 1열, 전체 지표 semantics 적용.
- [x] 온보딩 미리보기 혜택의 8px 축소 글자를 없애고 320px에서는 읽을 수 있는 2열 Wrap으로 전환.
- [x] 320px 홈 마이팀 최근 경기·팀 지표를 겹치지 않는 반응형 행으로 보정.
- [x] 320px 홈 순위 요약의 고정 열 너비와 안쪽 여백을 조정해 9px RenderFlex 넘침을 제거.
- [x] 일정 달력의 외곽선·마이팀 점·일반 경기 점·선택 채움과 범례를 일치.
- [x] 순위 상단을 승패 모두 포함한 가장 긴 `연속 흐름`으로 바꾸고 작은 화면 팀명 잘림과 화면 확인 시각 표현을 보정.
- [x] standalone 순위표의 320px 수치 열을 좁혀 모든 고정 열을 화면 안에 유지하고 마이팀은 행 색·왼쪽 강조선으로 구분.
- [x] 알림함의 최근 최대 50개 계약, 현재 표시 수, 목록/설정 독립 실패·재시도, 실패 중 unread `확인 불가`, 44px 선택 semantics를 연결.
