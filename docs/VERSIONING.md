...
- `0.0.65`: reuploads the `0.0.64` 경기별 push topic / pregame Live Activity / remote push self-test release state as TestFlight build `0.0.65+65` because App Store Connect already had build `64`; followed-game immediate push topics are narrowed to selected-game `*_GAME_<gameId>` topics while `summary` / `liveOnly` / `off` moments stay out of immediate FCM topics; the iOS Widget extension build settings now inherit Flutter `Generated.xcconfig` so Runner and Widget versions stay aligned.
