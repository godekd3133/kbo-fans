...
| 핵심 카드 | 핵심 타자 / 핵심 투수 요약 카드. 선수 프로필 이미지, 오늘 경기 주요 수치, 현재 경기 기준 생산/효율 라벨 노출 |
| 팀 비교 카드 | 선택 팀과 상대 팀의 득점/안타/타점/장타/볼넷/탈삼진을 한 줄 비교. optional field가 양쪽 모두 없으면 해당 비교 항목은 숨김 |
| 타자 카드 리스트 | 타순 정렬. 타수/득점/안타/타점 중심 카드. 선수 프로필 매칭 시 `선수 기록 보기` 진입점 노출. 원천이 제공한 경우 루타/장타/SLG/볼넷/사구/삼진/도루 chip 표시 |
| 투수 카드 리스트 | 등판 순서. 이닝/안타/삼진/사사구/자책 중심 카드. 선수 프로필 매칭 시 `선수 기록 보기` 진입점 노출. 원천이 제공한 경우 투구수/실점 chip 표시, 이닝 기준 게임 ERA/WHIP 파생 표시 |
| 상세 기록 펼침 | 각 선수 카드의 `상세 기록` 토글에서 오늘 세부 기록, 시즌 미니 지표, 멀티히트/득점 관여/무자책 같은 경기 맥락 badge를 표시 |
...
**인터랙션**:
- 팀 토글 탭 → 어웨이 ↔ 홈 전환
- 매칭된 선수 카드 탭 → 선수 상세 프로필로 이동
- `상세 기록` 탭 → 선수 카드 안에서 경기 세부 기록과 시즌 미니 지표 펼침/접기
- 매칭되지 않은 선수 row → 경기 기록만 표시하고 상세 진입 affordance는 숨김
- 시즌 OPS/ERA 같은 누적 지표는 박스스코어 response에서 계산하지 않는다. 선수 프로필 매칭으로 이미 확보한 시즌 지표가 있을 때만 상세 펼침의 미니 지표로 표시하고, 박스스코어 자체 파생은 경기 단위 SLG/루타/게임 ERA/WHIP로 제한한다.
...
| Moment 행 | 경기 시작 임박, 경기 시작, 득점, 안타, 홈런, 역전, 경기 종료, 라인업, 이닝 교대, 타석 |
...
- 푸시/로컬 알림 클릭은 외부 URL을 직접 열지 않고 앱 내부 route로 변환한다. `lineupOpened` / `lineup_opened`는 `/game/{gameId}?tab=lineup`, 경기 시작 임박/경기 시작/득점/안타/홈런/역전/이닝 교대/타석은 `/game/{gameId}?tab=relay`로 진입한다.
