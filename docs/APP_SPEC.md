...
| 캘린더 그리드 | 경기 있는 날은 얇은 live-red outline으로 날짜를 감싸고, 날짜 아래 4px dot을 둔다. 마이팀 경기 dot은 action blue, 일반 경기 dot은 muted gray로 표시하며 범례도 같은 모양을 사용 |
...
| 순위 테이블 | 승/패/무/승률/게임차/연속. 순위 순 정렬. 430px 이하에서는 공식 짧은 팀명을 사용하고 semantics에는 전체 팀명을 유지한다. 340px 이하에서는 수치 열·로고 간격을 함께 줄여 가로 넘침을 막는다 |
