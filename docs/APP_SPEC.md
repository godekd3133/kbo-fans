...
- 대량 보조 리소스는 리얼/시네마틱 이미지가 아니라 캐주얼 2.5D 스티커형 야구 일러스트를 사용한다. 홈, 경기 상세, 일정, 순위, 기록실, 설정, 온보딩에는 각 25개씩 총 175개 WebP 리소스를 48~54px 높이의 가로 레일로 배치해 화면 맥락을 가볍게 보강한다.
- 홈의 보조 섹션 카드는 3px 안팎의 얇은 팀/상태 컬러 rail로 containment를 주고, 카드 전체를 강한 색으로 덮지 않는다.
- 헤더 오른쪽처럼 행동이 없는 영역에는 장식성 알림 아이콘을 두지 않는다. 마이팀이 선택된 경우 팀 엠블럼을, 미선택이면 중립 야구 아이콘을 보여준다.
...
| KBO 브리프 | 마이팀 브리프 아래에서 그날 리그 전체의 강한 관전 포인트/확정 기록/핵심 경기 요약 |
| 오늘의 야구 | 오늘 경기 수, 라이브 경기 수, 당일 핵심 요약 |
| 빠른 콘텐츠 | 홈런왕, 오늘의 플레이어, 마이팀 순위 등 짧은 정보 카드 |

**UI 요소**:
| 요소 | 설명 |
|------|------|
| 헤더 | 앱 이름 + 날짜 + 실시간 표시(작은 상태 점 또는 `경기 중` 텍스트) + 선택된 마이팀 엠블럼 |
...
| 마이팀 경기 카드 | 상단 고정, 확대 카드. 진행 중 상태 배지는 필요 시 `LIVE`만 짧게 표시하고, 보조 라벨은 한국어를 우선한다. 이닝별 스코어(R/H/E/B) 포함. H/E/B 원천값이 없으면 해당 요약 행은 숨김 |
...
- 마이팀 브리프는 단순 지표 나열보다 `지금 내 팀 상황`을 먼저 판단하게 한다. 선택된 팀이 있으면 상태 pill, 한 줄 헤드라인(`5회초 3:2 리드`, `4:2 승리`, `오늘 18:30 vs 두산`, `오늘은 LG 경기 없음`), 짧은 상황 문장, 최근 3경기/순위/상태 지표, 기본 CTA 순서로 구성한다.
...
- 마이팀이 선택되어 있고 오늘 마이팀 경기가 live 상태이면 홈 진입 시 해당 경기를 기본 follow session target 으로 맞춘다. 별도 설정을 열지 않아도 홈 마이팀 경기 카드의 follow CTA는 같은 자리에서 `따라가는 중` 체크 상태로 바뀌고, `/push/register`의 `followedGameIds`도 조용히 갱신한다.
- 홈의 `따라가기` CTA를 명시적으로 누르면 같은 follow session 을 시작하고, OS permission / push sync 는 마이팀 자동 등록 경로를 보완하는 수동 재시도 경로로도 동작한다.
- 홈에서 진행 중인 경기 카드(마이팀 확대 카드, 오늘의 야구 spotlight, 일반 경기 카드)를 열면 경기 상세는 기본으로 `문자중계` 탭에서 시작한다.
- 마이팀 live 경기 카드의 박스 전체와 `중계 보기` CTA는 `문자중계` 탭으로 이동하면서 상세 상단 요약을 접어 문자중계 본문이 먼저 보이도록 focus 진입을 사용한다.
...
- 홈 UI는 `마이팀 브리프` 바로 아래에 `KBO 브리프` 라벨과 상태별 title을 표시하고, 기본 3개 카드 노출 후 4개 이상이면 확장한다.
