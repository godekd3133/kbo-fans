...
- 홈/일정/순위/기록실/선수 상세의 로딩, 에러, 데이터 상태 전환은 공통 fade + 미세 vertical slide를 사용해 갑작스러운 화면 교체를 줄인다.
- 경기 카드, 일정 카드, 순위 행, 선수/리더보드 행처럼 반복 리스트로 나타나는 정보는 짧은 등장 모션을 허용하되 스크롤 성능과 정보 밀도를 우선한다.
- 터치 가능한 경기/일정/기록실/설정 카드, 홈 보조 카드, 하단 탭, 필터/정렬 chip, 온보딩 팀 카드는 짧은 press scale + opacity 피드백을 사용한다.
...
- 실시간 점수와 경기 상세 스코어처럼 값이 바뀌는 숫자는 기존 레이아웃 크기를 유지하면서 value swap 모션을 적용한다.
...
- 생성 비주얼은 독립 배너나 삽입 이미지 카드처럼 보이게 두지 않는다. 정상 데이터 화면에서는 카드/표/헤더의 낮은 opacity background layer로 통합하고, 독립 `AppArtworkCard`는 온보딩, empty, retry처럼 화면 상태 자체를 설명하는 경우에만 사용한다.
- 대량 보조 리소스는 리얼/시네마틱 이미지가 아니라 캐주얼 2.5D 스티커형 야구 일러스트를 사용한다. 경기 상세, 일정, 순위, 기록실, 설정, 온보딩에는 각 25개씩 WebP 리소스를 48~54px 높이의 가로 레일로 배치해 화면 맥락을 가볍게 보강한다. 홈은 별도 가로 레일 대신 마이팀 브리프 카드의 낮은 opacity background layer와 실제 팀 로고/경기 행 안에 비주얼을 흡수한다.
...
- 월 헤더는 `scheduleTicketing` 생성 비주얼을 별도 스트립으로 분리하지 않고 헤더 카드의 낮은 opacity 배경으로 통합해 월 이동 컨트롤과 화면 맥락을 한 덩어리로 보여준다.
...
| `ArtworkCard` | 온보딩, 홈, 일정, 상세, 순위, 기록실, 설정 | 로컬 `assets/visuals/*` 생성 비주얼을 화면 맥락 카드로 노출. 로고/구단 엠블럼/읽을 수 있는 임의 텍스트는 이미지 안에 넣지 않는다 |
| `VisualResourceRail` | 홈, 경기 상세, 일정, 순위, 기록실, 설정, 온보딩 | 25개 단위 캐주얼 WebP 리소스를 작은 가로 스크롤 레일로 노출. 실사/리얼 톤이 아니라 둥근 2.5D 일러스트 톤을 유지한다 |
