...
- 대량 보조 리소스는 리얼/시네마틱 이미지가 아니라 캐주얼 2.5D 스티커형 야구 일러스트를 사용한다. 경기 상세, 설정, 온보딩에는 각 25개씩 WebP 리소스를 48~54px 높이의 가로 레일로 배치해 화면 맥락을 가볍게 보강한다. 홈은 별도 가로 레일 대신 마이팀 브리프 카드의 낮은 opacity background layer와 실제 팀 로고/경기 행 안에 비주얼을 흡수한다. 일정, 순위, 기록실 정상 화면은 캘린더/경기 목록/순위표/리더보드의 정보 밀도를 우선해 보조 비주얼 rail을 두지 않고, 빈/오류 상태에서만 생성 비주얼을 사용한다.
...
- 구조는 `{ title, subtitle, items[] }`이며, `items[]`는 `{ type, eyebrow, title, subtitle, route, gameId, teamIds[] }`를 가진다.
...
| 생성 비주얼 | 박스스코어 요약 상단과 공식 rows 미공개 빈 상태에 `boxscore_analytics` 생성 이미지를 사용한다 |
...
**목적**: 홈보다 한 단계 넓게 오늘 KBO 흐름을 가볍게 확인

```
┌─────────────────────────────────────┐
│  2026.06.19 기준                 ↻  │
│  뉴스                               │
│  오늘 경기, 순위, 기록 흐름을 짧게 정리 │
├─────────────────────────────────────┤
│ 전체  경기  순위  기록  마이팀         │
│                                     │
│  지금 KBO                            │
...
| 요약 카드 | `homeAggregate.kboBrief.title/subtitle`을 compact summary card로 표시 |
| 뉴스 카드 | `HomeKboBriefItem`과 마이팀 브리프를 카드로 변환. label, title, subtitle, source label, 이동 action을 포함 |
...
**인터랙션**:
- 하단 `뉴스` 탭 → `/news`
- 카드 탭 → item route 기준으로 경기 상세, `/standings`, `/records` 계열 화면으로 이동
...
→ HomeAggregate.kboBrief.items + HomeAggregate.myTeamBrief
```
...
**목적**: 마이팀과 알림 Moment를 사용자가 예측 가능하게 제어
...
- local/web 환경은 자동 권한 요청을 하지 않고, 설정 화면의 권한 확인 또는 경기 따라가기 같은 명시적 동작은 계속 수동 sync 진입점으로 유지한다.

**카피 원칙**:
- 설정 화면에는 `앱 밖 표면` 같은 내부 분류 설명 블록을 두지 않는다.
