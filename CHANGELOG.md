...
- 라이브 경기 상세의 게임·문자중계·박스스코어·라인업 준비를 병렬화하고, 이미 준비된 runtime 데이터를 우선 사용하도록 보강했습니다.
- Live Activity 동기화 사이클에 경기별 계측과 relay 병렬 prefetch를 추가해 라이브 경기 상태 수렴을 개선했습니다.

### Fixed

- KBO 원천이 느리거나 일부 경기 요청이 실패해도 경기 상세와 다른 경기의 동기화가 함께 굶지 않도록 마지막 사용 가능 데이터·명시적 상태·경기별 격리 경계를 보강했습니다.
- 대형 경기 데이터 응답의 JSON 처리와 캐시 쓰기 경량화를 적용해 화면 스레드의 불필요한 작업을 줄였습니다.

## [0.1.34] - 2026-09-13

### 새로워졌어요

- 일반 내비게이션 선택 상태의 전역 테마 토큰을 LIVE red에서 action blue로 정리했습니다.
- 홈 알림 아이콘에 저장된 미읽음 알림 수를 badge와 접근 가능한 tooltip으로 표시하고, 새 알림 저장·읽음 처리·앱 복귀에도 즉시 상태를 다시 반영하도록 다듬었습니다.
- 사장님이 컨펌한 새 홈 시안 1번을 기준으로 홈 헤더·마이팀 경기·오늘 경기·내 팀 기록의 시각 계층을 다시 맞추고 실제 KBO 팀 엠블럼을 표시하도록 다듬었습니다.
- 일정·순위·기록실·브리핑·설정·온보딩의 페이지 헤더와 하단 탐색 선택 상태를 같은 모바일 디자인 언어로 정리했습니다.
- 알림함·리그 리더보드·선수 상세·API 진단·업데이트 소식까지 화면별 시안을 정리하고 제목·설명·뒤로가기·상태 표시의 리듬을 같은 기준으로 맞췄습니다.
- 리더보드에서는 일반 지표 선택을 액션 블루로 분리하고, 선수 상세에서는 팀 엠블럼 색상과 대표 기록을 한 화면에서 이어서 읽을 수 있게 다듬었습니다.
- 알림함도 공통 페이지 헤더와 액션 블루 필터를 사용해, 읽지 않은 알림과 알림 준비 신호를 같은 흐름에서 빠르게 구분할 수 있게 다듬었습니다.
- 브리핑의 전체·경기·순위·기록·마이팀 목적 필터도 액션 블루로 정리해, 카드 안의 경기·순위·기록 의미색과 혼동하지 않도록 했습니다.
- 일정의 보기 모드·팀 필터·선택 날짜를 액션 블루로 분리하고, 경기 있는 날짜의 red 표시는 유지해 달력 상태를 더 빠르게 구분할 수 있게 했습니다.
- 기록실의 야수·투수 탭, 엔트리 필터, 정렬 칩과 리더보드 그룹·지표 선택도 액션 블루로 통일해 일반 선택과 기록 의미색을 분리했습니다.
- standalone 리더보드의 타자·투수 그룹 선택도 같은 액션 블루 규칙으로 맞췄습니다.
- 홈 순위 미리보기의 마이팀 강조색을 LIVE red에서 해당 구단의 readable team color로 바꿔, 팀 정체성과 경기 진행 상태를 구분했습니다.
- 홈 최근 5경기의 승·패·무와 연승·연패 색상을 positive green·negative red·action blue 의미로 정리해 LIVE 상태와 결과를 구분했습니다.
- 경기 상세 하이라이트의 `스크롤 / 플레이어 조작` 모드 선택을 액션 블루로 정리해 경기 상태색과 일반 재생 모드를 구분했습니다.
- 업데이트 소식의 `현재 설치됨` badge와 변경점 bullet을 액션 블루로 정리해 LIVE/오류 red와 정보 상태를 구분했습니다.
- 앱 시작 업데이트 prompt의 아이콘·bullet도 같은 액션 블루를 사용하도록 맞췄습니다.
- 여러 화면의 데이터 로딩 indicator를 액션 블루로 통일해, 대기 상태가 LIVE 경기나 오류처럼 보이지 않게 정리했습니다.
- pull-to-refresh indicator도 액션 블루로 통일해, 새로고침 대기와 LIVE 경기 상태를 구분했습니다.
- 브리핑 화면의 pull-to-refresh도 액션 블루로 맞춰 전체 refresh 상태 규칙을 완성했습니다.
- 설정의 화면 모드 선택도 액션 블루로 분리해, 현재 선택과 LIVE·오류 상태가 같은 색으로 보이지 않게 정리했습니다.
- 설정의 푸시 알림 프리셋 선택도 마이팀 색상과 분리된 액션 블루를 사용하도록 정리해, 팀 대상 정보와 일반 선택 상태를 구분했습니다.
- 경기 상세 스코어 탭의 정상 이닝 표에서 장식 이미지를 걷어내고 득점·안타·실책·사사구 숫자가 먼저 읽히도록 다듬었습니다.
- 경기 상세 라인업 탭에서 선발 타순과 선발 투수를 먼저 보여주고, 최근 흐름·맞대결 비교는 그 다음에 읽도록 순서를 정리했습니다.
- 문자중계의 회차·장면 필터 선택을 action blue로 통일하고, 득점·홈런·교체 이벤트의 강조색은 정보 의미로 유지했습니다.
- 박스스코어의 LIVE 집계중 상태를 팀 요약 헤더의 `LIVE` pill로 먼저 보여주고, 공식 기록 전 단계임을 숫자 타일과 함께 확인할 수 있게 했습니다.
- API 진단 중에는 다시 진단 버튼을 잠그고 진행 spinner를 표시해, 빠른 연속 탭으로 진단 요청이 겹치지 않도록 다듬었습니다.
- 기록실의 리그·팀 기록 새로고침도 요청 중 버튼을 잠그고 진행 상태를 표시해, pull refresh·header·오류 재시도가 같은 요청을 중복 실행하지 않도록 다듬었습니다.
- 순위표 새로고침도 요청 중 버튼을 잠그고 진행 상태를 표시해, header·pull refresh·오류/빈 상태 재시도가 중복 실행되지 않도록 다듬었습니다.
- 일정 refresh도 월별·시즌 매치업 fan-out 요청 중 재진입을 막아, pull refresh와 오류 retry가 같은 provider를 중복 무효화하지 않도록 다듬었습니다.
- 브리핑 refresh도 header·pull refresh·오류 retry를 하나의 in-flight 요청으로 묶고 진행 spinner를 표시하도록 다듬었습니다.
- 알림함 목록과 설정 loader도 동시 refresh·retry를 하나의 in-flight 요청으로 합쳐, 오래된 응답이 최신 inbox 상태를 덮지 않도록 다듬었습니다.
- 선수 상세의 pull refresh와 오류 retry도 하나의 in-flight 요청으로 묶어 player provider 중복 호출을 막았습니다.
- API 진단 카드가 상태·detail·소요시간·note를 하나의 접근성 요약으로 읽히도록 다듬었습니다.
- 업데이트 소식 화면과 앱 시작 업데이트 prompt의 현재 버전·변경점 묶음을 하나의 접근성 요약으로 읽히도록 정리하고, 다이얼로그 전환이 끝난 뒤에도 접근성 회귀가 안정적으로 검증되도록 보강했습니다.
- 설정 화면 모드 선택을 중복 버튼이 아닌 단일 접근성 제어로 정리해 `시스템 모드`·`라이트 모드`·`다크 모드`를 한 번씩만 탐색하도록 다듬었습니다.
- 박스스코어의 매칭된 선수 행이 스크린리더에서도 `선수 기록 보기` 탭 동작을 제공하도록 보강했습니다.
- 경기 상세 상단의 `뒤로` 버튼도 중첩 semantics를 제거해 한 번만 탐색되고 동일한 복귀 동작을 실행하도록 정리했습니다.
- 공통 `AppPressable`의 명시적 접근성 label을 semantic boundary로 격리해, 페이지 헤더의 뒤로가기 label에 제목·설명이 합쳐지지 않도록 보강했습니다.
- 일정의 이전 달·다음 달·오늘 이동 아이콘도 단일 접근성 action으로 정리해 월 제목과 중복 탐색되지 않도록 보강했습니다.
- 홈 순위 미리보기 행도 외부·내부 button 중첩을 제거해 `팀 순위 전체 보기` action을 한 번만 탐색하도록 정리했습니다.
- 온보딩 팀 선택 카드의 selected 상태도 공통 `AppPressable` 하나에 연결해 선택 semantics 중복을 줄였습니다.
- 기록실 팀 상세의 `뒤로` 버튼도 단일 summary/action node로 정리해 제목과 중복 탐색되지 않도록 보강했습니다.
- 온보딩 편집 모드의 `뒤로` 버튼도 동일한 단일 summary/action semantics로 정리했습니다.
- 공통 페이지 헤더를 explicit child semantics container로 분리해, 뒤로가기 action과 페이지 제목·설명이 같은 요약 label로 합쳐지지 않도록 보강했습니다.
- 홈 헤더의 `알림함`·`기록 검색` 아이콘도 부모 Tooltip이 아닌 실제 button label과 tap action으로 직접 노출되도록 보강했습니다.
- 게임 상세 영상의 `스크롤`·`플레이어 조작` mode chip도 selected 상태를 보조기술에 전달하도록 보강했습니다.
- 기록실 `마운드 체크` category 아이콘을 LIVE red가 아닌 action blue로 분리해 투수 기록과 경기 진행 상태를 혼동하지 않도록 정리했습니다.
- 홈 헤더의 알림·검색 아이콘 hit target을 최소 44px 기준으로 맞춰 Material 기본 `48×48` 조작 영역을 유지하도록 보강했습니다.
- 순위표와 기록실 팀 상세의 시즌 선택 surface도 최소 44px 높이를 명시해 작은 화면에서 선택 영역이 눌리지 않도록 보강했습니다.
- 설정의 탭 가능한 섹션 row에서 `AppPressable` 바깥의 중복 button semantics wrapper를 제거해 각 row가 하나의 접근성 action으로만 노출되도록 정리했습니다.
- 예정 경기의 홈·브리핑 quick item이 원천의 `0 : 0` placeholder를 실제 점수로 보이지 않게 `vs`와 미제공 score로 정리했습니다.
- 홈 최근 5경기 오류·빈 상태의 잘못된 `순위 보기` CTA를 실제 결과를 확인할 수 있는 `일정 보기`로 연결했습니다.
- 홈 예매 quick item의 ISO timestamp를 `9월 6일 11:00 KST` 형식으로 정리하고, 예정 인사이트 score strip도 `예정 · 팀 vs 팀 · 시작 시각` 순서로 다듬었습니다.

### 고쳤어요

- LIVE 경기에서 미확정 점수가 중앙과 팀 정보에 중복 표시되지 않도록 정리했습니다.
- 경기 상세 scorebug에서 정상 데이터 화면의 배경 장식보다 팀 로고·점수·상태가 먼저 읽히도록 조정했습니다.
- 팀 컬러 rail이 긴 선수 상세 콘텐츠의 높이를 잘못 계산해 렌더 assert를 만들 수 있던 공통 surface 레이아웃을 콘텐츠 높이 기준으로 고쳤습니다.
- 알림함·API 진단·업데이트 소식을 주소로 직접 열어도 뒤로가기가 알맞은 상위 화면으로 돌아가도록 보강했습니다.
- 운영 release health gate가 각 API endpoint의 응답 시간을 함께 출력하도록 보강해, HTTP 200만으로는 구분하기 어려운 cold/warm 성능 차이를 배포 후 확인할 수 있게 했습니다.
- release health gate에 선택형 `RELEASE_API_HEALTH_PERFORMANCE=true` 모드를 추가해 같은 endpoint 묶음을 cold/warm 순서로 두 번 확인하고, 각 단계의 응답 시간을 비교할 수 있게 했습니다.

### 빨라졌어요

- 홈 aggregate와 홈 scoreboard가 독립적으로 조회할 수 있는 KBO 데이터를 순서대로 기다리지 않고 함께 준비하도록 다듬고, 홈에 필요한 기록 지표만 따로 조회해 캐시가 비어 있는 첫 응답의 대기 시간을 줄였습니다.
- 홈 aggregate가 현재일 scoreboard를 이미 받은 경우에는 월간 일정 조회에서 같은 Main 목록을 다시 요청하지 않도록 정리했습니다. 일반 일정 화면의 현재일 보정은 그대로 유지됩니다.
- 홈·일정·스코어보드 서비스가 같은 프로세스 안에서 짧은 KBO 원천 응답 cache와 singleflight를 공유해, 같은 월간 일정과 Main 목록을 연속으로 중복 조회하지 않도록 다듬었습니다.
- 홈에서 마이팀 최근 결과가 현재 월만으로 5경기에 부족한 경우 이전 월 일정도 standings·records 응답과 겹쳐 준비하도록 다듬었습니다. 최근 결과가 충분한 경우에는 이전 월을 추가 조회하지 않습니다.
- 기록실 전체 요약에서 같은 OPS 원천 페이지를 두 번 읽지 않고 OPS와 호환용 OPS 지표를 한 번에 해석하도록 다듬어 기록 화면의 첫 로딩 대기를 줄였습니다.
- 기록실 요약에서 리더보드 상세로 바로 이동할 때 최근 8초 안의 같은 시즌 page 응답을 재사용해 원천 재요청을 줄였습니다.
- 홈에서 이미 확인한 현재 시즌 기록을 기록실 전체 요약이 다시 조회하지 않고, 부족한 기록 그룹만 이어서 조회하도록 다듬었습니다.
- 기록실 전체 요약의 7개 고유 원천 page를 한 지표씩 기다리지 않고 함께 조회하도록 다듬었습니다.
- 기록실 원천 page 중 하나가 실패할 때 나머지 page의 timeout까지 오류 응답이 기다리지 않도록 다듬었습니다.
- 같은 선수·시즌의 player detail 요청이 동시에 들어와도 KBO 원천을 한 번만 조회하도록 다듬었습니다.
- 타입이 생략된 선수 상세의 결과를 실제 hitter/pitcher 타입 cache에도 연결해, 같은 상세를 명시 타입으로 다시 열 때 원천을 중복 조회하지 않도록 다듬었습니다.
- 같은 팀·시즌의 team stats 요청이 동시에 들어와도 KBO 원천을 한 번만 조회하도록 다듬었습니다.
- 팀 타격·투수 기록 중 한 원천이 실패할 때 다른 원천의 timeout까지 기다리지 않고 오류를 빠르게 표시하도록 다듬었습니다.
- current 시즌 팀 기록은 hitter/pitcher 화면을 각각 불필요하게 season POST하지 않고 선택된 GET page를 재사용하도록 다듬었습니다.
- 현재 시즌 순위도 최초 GET 응답이 요청 시즌으로 검증되면 불필요한 season POST를 생략하고, selector가 다르거나 검증할 수 없을 때만 기존 POST 경로를 사용하도록 다듬었습니다.
- 팀 선수 목록에서 entry 선수의 profile과 시즌 통계 원천을 순차로 기다리지 않고 기존 worker 상한 안에서 함께 준비하도록 다듬었습니다.
- 선수 상세에서 profile과 시즌 누적 통계를 순차로 기다리지 않고 함께 준비하도록 다듬었으며, 한 page 실패 시 다른 page timeout을 기다리지 않도록 했습니다.
- 팀 선수 검색 fan-out에서 한 position group이 실패해도 다른 group의 timeout까지 오류 응답이 기다리지 않도록 다듬었습니다.
- 팀 기록 화면에서 선수 목록 원천이 실패할 때 팀 통계 원천 timeout까지 오류 응답이 기다리지 않도록 다듬었습니다.
- 백그라운드 LIVE/FINAL 상세 준비에서 relay 인증 요청을 먼저 안전하게 마친 뒤 boxscore와 lineup을 함께 수집하도록 다듬었습니다.
- 백그라운드 상세 warm에서 이미 확인한 game summary를 문자중계 경로가 재사용하도록 해 같은 경기의 scoreboard 조회를 중복하지 않도록 다듬었습니다. 일반 relay API와 game identity가 맞지 않는 payload는 기존 재조회 경로를 유지합니다.
- 백그라운드 warm에서 lineup이 내부적으로 요청하는 boxscore에도 같은 force-refresh 의도를 전달해, worker의 직접 boxscore 요청과 같은 SingleFlight로 합쳐지도록 다듬었습니다.
- 게임 순간 push의 durable outbox target을 먼저 claim한 뒤 FCM 전송을 병렬화해, 팀별·경기별 topic fan-out 대기를 줄였습니다.
- outbox target claim과 전송 결과 반영도 batch 처리해 한 이벤트마다 registry 파일 lock/write가 target 수만큼 반복되지 않도록 다듬었습니다.
- push registry가 다른 요청의 파일·thread lock을 무기한 기다리지 않고 bounded wait 후 503으로 실패하도록 다듬었습니다.
- Live Activity 전송에서 모든 target이 stale claim으로 탈락한 경우 불필요한 빈 registry 반영을 건너뛰도록 다듬었습니다.
- Live Activity sync worker가 등록 상태를 확인할 때 push registry를 세 번 읽지 않고 한 번의 일관된 snapshot으로 판단하도록 다듬었습니다.
- 경기 상세의 lineup snapshot이 없는 첫 요청에서는 lineup과 boxscore 원천을 함께 조회해 선발 정보 조립까지의 대기 시간을 줄였습니다.
- lineup 원천이 먼저 실패할 때 boxscore 원천 timeout까지 오류 응답이 기다리지 않도록 다듬었습니다.
- current/live boxscore는 경기 상태 확인과 공식 boxscore 원천을 함께 시작해 두 원천 대기가 직렬로 누적되지 않도록 다듬었습니다. 과거 경기의 snapshot-first 경로는 유지됩니다.
- lineup snapshot miss에서는 lineup·boxscore·선발 Main 목록을 함께 준비해 선발 정보 조립 전의 추가 Main 대기를 줄였습니다.
- relay warm 직후 live-context boxscore가 완전한 relay cache를 먼저 재사용하도록 해 동일 경기의 relay crawler 재호출을 줄였습니다. cache miss에서는 기존 direct relay 원천 fallback을 유지합니다.
- current/live 경기 상세의 lineup·boxscore·문자중계가 historical immutable snapshot 파일을 불필요하게 읽지 않고 runtime cache와 현재 원천만 사용하도록 다듬었습니다. 과거 경기의 snapshot-first 동작은 유지됩니다.
- 경기 요약·일정·lineup이 같은 날짜의 Main 목록을 짧은 원천 cache에서 함께 읽도록 연결해 상세 전환 중 중복 조회를 줄였습니다.
- 경기 상세 요약도 schedule과 Main 목록을 함께 조회해 상세 진입 초기에 두 원천 대기가 직렬로 누적되지 않도록 다듬었습니다.
- 경기 상세의 schedule 원천이 실패하면 schedule fallback을 다시 호출하지 않고 명시적인 upstream unavailable 상태를 반환하도록 다듬었습니다. 실제 경기 없음은 기존 404 경로를 유지합니다.
- boxscore가 live-context fallback을 만들 때도 같은 Main 목록 source를 재사용해 scoreboard 이후의 중복 조회를 줄였습니다.
- full scoreboard도 schedule과 Main 목록을 함께 조회해 경기별 상세 보강을 시작하기 전의 순차 대기를 줄였습니다.
- compact scoreboard도 schedule과 Main 목록을 함께 조회해 위젯용 경기 선택이 두 원천 조회 시간을 합산해 기다리지 않도록 다듬었습니다.
- current/future scoreboard 요청은 historical snapshot 파일을 불필요하게 확인하지 않고 process/runtime cache와 현재 원천 경계를 바로 사용하도록 다듬었습니다.
- current/future 일정·순위·기록실·팀 기록·선수 데이터도 historical snapshot 파일을 매번 확인하지 않고 현재 원천 경계를 바로 사용하도록 다듬었습니다.
- 홈·스코어보드·compact 중 한 원천이 실패할 때 관련 없는 sibling 요청의 종료까지 응답이 붙잡히지 않도록 다듬어, 실패 상태도 더 빠르게 표시되게 했습니다.
- 병렬로 준비하는 records·lineup·team stats·team records·player detail 원천도 뒤쪽 요청이 먼저 실패하면 앞쪽 timeout까지 기다리지 않고 오류를 표시하도록 다듬었습니다. current/future failure visibility와 전체 응답 deadline은 유지됩니다.
- 검증된 과거 문자중계 상세 snapshot이 있으면 경기 요약 원천을 다시 조회하지 않고 바로 표시하도록 다듬었습니다.
- 팀 선수 목록은 한국어 공식 roster를 먼저 사용하고, 실제 roster가 비어 있을 때만 영문 검색을 fallback으로 사용하도록 해 불필요한 원천 요청을 줄였습니다.
- 한국어 roster에 이미 있는 목록용 프로필 정보는 다시 선수 상세 페이지에서 읽지 않고, entry 선수에게 필요한 시즌 누적 통계만 준비하도록 다듬었습니다. 선수 상세 화면의 전체 프로필 조회는 그대로 유지됩니다.
- 팀 선수 목록에서 확인한 player type을 짧게 재사용해, 목록에서 바로 연 선수 상세가 hitter/pitcher 판별 페이지를 다시 조회하지 않도록 다듬었습니다.
- player type을 알 수 없는 선수 상세도 판별 단계에서 이미 올바른 Basic 페이지를 받은 경우 같은 페이지를 다시 읽지 않도록 다듬었습니다.
- 현재시즌 팀 선수 통계는 선수별 누적 페이지를 반복 조회하지 않고 KBO 팀 필터 bulk 기록 3개를 조합하도록 다듬었으며, bulk에서 빠진 선수만 기존 개별 통계로 보충합니다.
- 병렬 KBO crawler가 worker마다 독립적인 HTTP session을 사용하도록 다듬어 WebForms cookie·요청 상태가 서로 섞이지 않게 했습니다. 인증 relay session은 기존 전용 경계를 유지합니다.
- 병렬 KBO 요청이 동시에 실패할 때 circuit breaker의 실패 횟수가 누락되지 않도록 보호해, 같은 upstream 장애에 재요청이 몰리는 시간을 줄였습니다.
- 같은 경기의 하이라이트 요청이 동시에 들어와도 YouTube 검색·제목 조회가 중복 실행되지 않도록 결과를 함께 사용하게 다듬었습니다.

## [0.1.33] - 2026-09-08

### Fixed

- Android release 빌드에서 `home_widget` Kotlin 모듈과 앱의 JVM target이 달라 컴파일이 실패하던 문제를 수정했습니다.
- 0.1.32의 홈·경기 상세·기록실 리뉴얼과 백엔드 데이터 신뢰성 개선을 새 배포 빌드에 그대로 포함했습니다.

## [0.1.32] - 2026-09-08

### 새로워졌어요

- 홈에서 내 팀의 경기 상황을 먼저 보고 전 경기와 팀 기록으로 이어집니다. 경기 없는 날에는 다음 일정으로 바로 이동할 수 있습니다.
- 경기 상세에 점수와 확인된 이닝 기록을 읽어주는 `경기 한눈에`, 문자중계에 `핵심 장면` 모아보기를 추가했습니다.
- 기록실·리더보드·선수 상세에서 9개 지표의 뜻과 읽는 법을 확인하고 같은 팀·같은 시즌 선수 두 명의 주요 지표를 비교할 수 있습니다.
- 알림을 `결과 중심`, `주요 순간`, `직접 설정`으로 선택할 수 있습니다.

### 고쳤어요

- 과거 시즌을 선택하고 팀 기록으로 들어가도 현재 시즌으로 돌아가지 않습니다.
- 취소·종료·진행 중 경기를 다음 예정 경기로 안내하거나, 미제공 점수를 0점으로 해석하던 브리핑을 수정했습니다.
- 경기 없는 날 다음 달로 이동할 때 월초 예정 경기를 건너뛰지 않게 했습니다.
- 박스스코어에서 동률을 한 팀의 우위로 표시하거나 미제공 장타를 0으로 비교하던 문제를 수정했습니다.
- 선수 비교 선택기의 이름 잘림과 리그 기록 로딩 후 내 팀 바로가기가 밀리는 문제를 수정했습니다.
- 브리핑의 예매 시각을 읽기 쉬운 한국시간으로 바꾸고 예상·공식 여부와 실제 근거를 구분했습니다. 점수 미확정·종료 브리핑에 라이브 라벨을 붙이지 않습니다.

### 작게 다듬었어요

- 온보딩의 팀 선택과 가치 설명을 간결하게 하고, 브리핑 필터·제목·계산 근거와 생성 날짜를 읽기 쉽게 정리했습니다.
- 홈 헤더에 실제 야구 날짜를 표시하고, 하단 탐색과 기록 설명·비교 창의 디자인을 다듬었습니다.
...
- 현재 경기의 정상 박스스코어와 라인업도 앱에 잠시 저장해 네트워크가 불안정할 때 마지막으로 확인한 내용을 표시하도록 보강했습니다.

### Fixed

- 박스스코어 또는 공개된 라인업의 첫 요청이 일시적으로 실패해도 탭을 열어 둔 상태에서 자동으로 다시 요청하도록 수정했습니다.
- 저장된 박스스코어와 라인업을 보여줄 때 최신 갱신이 지연된 상태임을 화면에 안내하도록 추가했습니다.

## [0.1.30] - 2026-09-02

### Changed

- 문자중계 응답을 backend의 process/runtime cache와 앱의 제한된 로컬 cache에서 재사용할 수 있도록 보강했습니다.

### Fixed

- 경기 요약 요청이 오래 걸리거나 첫 문자중계 요청이 실패해도, 탭을 열어 둔 상태에서 문자중계를 자동으로 다시 요청하도록 수정했습니다.
- 네트워크가 일시적으로 끊기면 최근 60초 안에 저장한 문자중계를 표시하고, 화면에 최신 데이터가 아님을 안내하도록 수정했습니다.
...
- 완료된 과거 경기·일정·순위와 과거 시즌 선수 기록은 정상 로컬 cache가 있으면 시간 만료만으로 백그라운드 재조회하지 않고 즉시 재사용하도록 변경
- 현재 날짜·현재 시즌·LIVE 데이터는 기존 fresh-first 정책을 유지하고, 사용자가 명시적으로 요청한 경기·스코어보드 강제 갱신은 과거 cache를 우회하도록 유지

### Fixed

- 종료 경기 문자중계의 득점 요약, 공식 기록이 비어 있는 박스스코어, 한쪽이 비어 있는 라인업이 완성된 과거 데이터처럼 장기 저장될 수 있던 경계를 보정
- 과거 경기 cache가 다른 경기 ID, 다른 월, 다른 시즌 또는 1위부터 시작하지 않는 기록 payload를 영구 재사용하지 않도록 식별자·완성도 검증을 보강

## [0.1.27] - 2026-08-28

### Changed

- 기존 Lightsail 운영 환경과 secret를 덮어쓰지 않고 backend runtime만 갱신할 수 있도록 배포 옵션을 보강

### Fixed

- 일정에서 경기 상세로 들어갈 때 최신 경기정보 API가 늦어도 일정에 표시된 팀·상태·점수 요약을 먼저 보여주도록 보정
- 라이브 경기의 문자중계 원문이 늦게 도착해도 현재 경기 요약을 먼저 표시하고, 원문은 화면 안에서 이어서 불러오도록 개선
- 종료 경기의 문자중계 원문 cold 조회가 오래 걸려도 득점 요약을 먼저 반환하고, 상세 원문은 백그라운드에서 준비하도록 개선
- 라인업 응답이 선수 프로필 보강까지 기다리며 늦어지던 경로를 분리해, 기본 라인업과 선발 정보를 먼저 표시하도록 보정
- 박스스코어 공식 기록이 선수 프로필 보강까지 기다리지 않고 타자·투수 rows를 먼저 표시하도록 보정
- 경기 전 라인업이 아직 공개되지 않은 경우 불필요한 원천 조회를 시작하지 않고 즉시 공개 전 상태를 표시하도록 개선

## [0.1.26] - 2026-08-13

### Changed

- 앱과 backend 화면 데이터 요청이 전체 시간 예산 안에서 종료되고, 일시적인 네트워크·서버 응답만 제한적으로 재시도하도록 보강
- 라이브 스코어보드 갱신을 push·Live Activity 전송과 분리해 알림 전송이 느려도 경기판 최신성이 유지되도록 개선
- 모바일 기록 영역에서 순위표와 선수 기록을 한 번에 전환할 수 있도록 정보 구조를 정리
- 좁은 화면과 큰 글자에서 홈·일정·설정·경기 상세의 내용이 잘리지 않도록 세로 재배치와 상태 안내를 보강

### Fixed

- 확인되지 않은 점수·최근 결과·과거 snapshot을 공식 데이터처럼 재사용하거나 실제 0으로 표시할 수 있던 경계를 보정
- 같은 원천 데이터를 동시에 요청할 때 대기열이 끝없이 늘어나거나 동일 크롤링이 반복될 수 있던 경로를 제한

## [0.1.25] - 2026-08-13

### Changed

- 앱의 화면 데이터 GET에 전체 25초 마감시간과 실제 요청 취소를 적용하고, 연결/전송 timeout과 HTTP 408·429·502·503·504만 한 번 재시도하도록 안정화. 서버가 `Retry-After`를 보내면 남은 전체 예산 안에서 이를 우선한다.
- backend 화면 데이터 요청을 15초 전체 예산과 제한된 동시 작업 수로 격리하고, 중계·푸시 주기를 느리게 조정해도 라이브 스코어보드 갱신은 기본 5초로 유지되도록 운영 경로를 분리
- 모바일 `기록` 영역에 `순위표 / 선수 기록` 전환을 추가해 핵심 순위 기능을 한 번의 탭으로 찾고 양방향으로 오갈 수 있게 개선
- 설정의 알림 토글을 항목명·설명·켜짐 상태가 하나로 읽히는 조작으로 합치고, 320px·240% 글자에서 화면 모드·알림·세부 항목이 생략되지 않도록 세로 재배치
- 설정에 사용자 동작으로만 여는 알림 권한 확인 CTA를 추가해 자동 권한 팝업 없이 OS 권한을 복구할 수 있게 개선
- 일정 경기 카드와 홈 최근 경기 행을 320px·240% 글자에서 내용 높이에 맞게 재배치하고, 홈 최근 5경기는 실제로 확인된 마이팀 결과만 표시하도록 정보 밀도를 줄임
- 홈 최근 흐름은 실제로 확인된 경기 요약만 결과 버블로 보여주고, 데이터가 없는 팀은 `최근 결과 없음`으로 구분해 연승 수치나 팀 ID로 만든 가짜 승·패를 제거
- 마이팀 브리프의 요약 영역과 `경기 일정`/`팀 기록` 버튼을 분리해 카드 전체 탭과 독립 액션이 겹치지 않도록 개선
- 데이터 브리핑은 같은 선수·경기·순위 사실의 제목 변형만 정리하고, 서로 다른 기록 지표와 경기 흐름은 숨기지 않도록 사실 단위 중복 제거를 보강
- 웹 미리보기에서 개발용 플로팅 콘솔을 숨기고, 경기·리더보드·선수 상세의 주소와 뒤로가기를 실제 화면 흐름에 맞게 정리
- 280/320px 화면과 1.3x~2.4x 큰 글자에서 경기 상세, 홈 오늘 경기, 순위, 기록실, 알림, 업데이트 소식이 내용에 맞춰 확장되도록 보강
- 리더보드·선수 기록·일정 오류 화면에 화면 안 `다시 시도`를 제공하고, 기록실 지표 rail에는 현재 위치를 알 수 있는 스크롤 표시를 추가
- 홈 aggregate가 비어도 공식 점수가 확인된 종료 경기만 보조하고, 중단·진행 경기의 미확정 점수는 `–`로 유지해 데이터가 없는 값을 실제 0으로 보이지 않게 보강
- API 입력 검증, 과거 snapshot 식별자 검증, 손상된 relay·lineup 응답 경계를 추가해 잘못된 요청과 다른 경기·시즌 데이터 재사용을 차단
- Live Activity와 push registry의 오래된 상태·backoff·heartbeat 쓰기를 정리하고, APNs·하이라이트 조회를 재사용·병렬화해 장시간 동기화 부담을 줄임
- HTTPS 배포 환경에서 ACM 인증서와 API 도메인 일치 여부를 확인하도록 배포 설정과 사전 점검을 강화

### Fixed

- KBO 응답, 서버 공유 lock, 동일 데이터 중복 조회가 오래 걸릴 때 로딩이 끝없이 남거나 후속 요청이 대기열에 누적되던 경로를 명시적 503/504와 재시도로 종료하도록 보정
- 시즌 매치업 9개월 일정과 홈·경기상세 중복 새로고침이 요청 wave마다 대기시간을 더하지 않도록 전체 25초 coordinator deadline으로 제한
- 과거 일정 snapshot에 예정·진행·중단 경기 또는 점수가 빠진 종료 경기가 남아도 영구 일정으로 재사용하지 않고 원천을 다시 확인하도록 보정
- 같은 날짜·팀의 동시 홈/순위/기록 요청을 한 번의 원천 조회로 합치고, 만료된 push·Live Activity 등록은 새 등록이 없어도 발송 대상에서 즉시 제외하도록 보정
- current/future 경기에서 과거 detailed snapshot을 잘못 재사용하거나 다른 경기·시즌·선수의 snapshot을 반환하던 경계를 보정
- 운영 secret이 없는 상태에서 push diagnostics가 공개되던 경로와 Python 날짜 범위를 벗어난 고카디널리티 요청 경로를 차단
- Live Activity worker 재생성으로 APNs 일시 실패 backoff가 초기화되던 문제와 같은 heartbeat를 매번 registry 파일에 다시 쓰던 문제를 보정
...
- 등록된 기기가 없을 때 백그라운드 Live Activity 동기화가 불필요한 KBO 조회와 저장소 기록을 반복하지 않도록 줄임
- 리더보드 오류 화면에서 화면을 나가지 않고 `다시 시도`로 즉시 복구하고, 320px·240% 글자에서 문자중계 안내 카드가 내용에 맞게 확장되도록 개선
- 선수 기록 오류도 화면 안 `다시 시도`로 복구하고, 1.5x 순위 행을 적응형 큰 글씨 레이아웃으로 전환해 긴 팀명·연속 기록이 겹치지 않도록 개선
- 알림 갱신 안내, 선수 메타정보, 순위표 접근성 label을 좁은 화면·큰 글씨 기준으로 다듬어 정보가 잘리지 않도록 개선
- 잘못된 game/team/player/month/season 요청을 crawler 앞에서 거절하고, push diagnostics는 운영 secret 미설정 시 공개하지 않도록 보강
- 홈 집계가 일시적으로 최근 결과를 비워도 scoreboard에서 공식 점수가 확인된 종료 경기만 보조해 `최근 결과 없음`으로 잘못 보이는 상태를 줄임
- 오래된 registry/outbox/live scoreboard 상태를 자동 정리하고, APNs 일시 실패에는 token별 backoff를 적용해 저장소·네트워크 retry 비용을 줄임
- 5초 동기화 heartbeat가 같은 상태일 때 registry 파일을 매번 다시 쓰지 않도록 줄여 장시간 실행 시 EFS 파일 잠금·쓰기 부담을 낮춤
- 중단된 경기라도 양 팀의 공식 점수가 확인되면 홈 오늘 경기 표에 실제 점수를 보여주고, 점수 미확정일 때만 `–`로 유지
- 홈 오늘 경기 행을 320px·240% 글자 크기에서 높이와 팀 정보 배치를 자동 확장해 구장·팀명·상태가 잘리지 않도록 개선
- Live Activity sync worker가 매 tick 같은 서비스 인스턴스를 재사용해 일시적인 APNs 실패 backoff가 5초 주기마다 초기화되지 않도록 보강
- HTTPS 배포는 ACM 인증서와 일치하는 `API_DOMAIN_NAME` 없이는 시작하지 않도록 체크리스트와 배포 스크립트를 맞춤
- 경기 하이라이트의 YouTube oEmbed 제목 조회를 최대 3개씩 병렬화해 네트워크가 느릴 때 첫 상세 화면의 대기 시간을 줄임
- Live Activity APNs 연결을 token마다 새로 만들지 않고 sender 수명 동안 재사용하며, 여러 기기 전송은 최대 4개씩 제한해 경기 업데이트가 등록 기기 수만큼 직렬 지연되지 않도록 개선
- 웹 미리보기에서는 개발용 플로팅 콘솔이 경기 카드와 설정을 가리지 않도록 숨기고, 리더보드·선수 상세를 직접 열어도 한국어 `뒤로` 버튼으로 기록실로 돌아갈 수 있게 개선
- 웹에서 일정·홈 카드로 상세 화면을 열면 주소도 상세 route로 함께 바뀌어 새로고침·공유 뒤에도 같은 경기를 다시 열 수 있게 개선
- 클라이언트 진단 전송량을 source별로 제한하고, 잘못된 선수 역할·손상된 과거 기록 snapshot을 조용히 사용하지 않도록 보강
...
- 원천 점수를 확인하지 못한 LIVE 경기의 홈·스코어·문자중계가 0:0으로 보이던 경로를 `–`/확인 중 상태로 통일하고, 종료 경기를 예정 경기로 되돌리는 backend fallback을 수정
- 선수 상세의 빈 메타정보 pill과 과도한 client metrics 본문을 제거·제한하고, HTTPS 운영 ALB의 평문 HTTP 요청을 HTTPS로 돌리도록 보강
- 종료·중단 경기와 미확정 점수를 실제 값처럼 보이게 하던 fallback을 `null`/`–` 상태로 보존하고, 90일 runtime state·7일 pending outbox·14개 live scoreboard 날짜 cap으로 장기 운영 파일 증가를 제한
...
- 과거 팀·선수·일정·스코어보드 snapshot이 다른 팀·선수·시즌·월·경기 날짜의 payload를 재사용하지 않도록 요청 식별자와 기본 shape를 확인하고, 손상된 relay·lineup snapshot은 사용자에게 500 대신 정상 fallback 경계로 보정
- API의 날짜·월 입력이 Python 날짜 범위 전체를 crawler key로 쓰지 않도록 KBO 지원 연도 범위를 적용해 `0001`·`9999` 같은 요청을 원천에서 422로 거절
