...
- 홈과 일정에서 경기 카드를 누르면 사전 갱신과 대량 선수사진 준비를 기다리지 않고 상세 화면을 즉시 열고, 최신 데이터는 화면 안에서 이어서 갱신하도록 변경
- 긴 문자중계는 현재 화면에 필요한 카드만 만들어 많은 타석이 쌓인 경기의 첫 표시와 스크롤 부담을 줄임
- 기록실 필터·정렬·타자/투수·지표 선택 상태를 스크린리더에 전달하고, KST 자정이 지나면 홈과 일정이 앱 재실행 없이 새 경기일로 전환되도록 개선. 순위와 기록실은 현재 시즌을 보고 있을 때만 새해의 새 시즌으로 이동하고 직접 고른 과거 시즌은 유지
- 현재 경기·현재 시즌 응답을 기기 저장소에 반복 저장하지 않고 과거 데이터 cache도 최대 2 MiB 안에서 정리해 장기 사용 시 저장 공간 증가를 제한
- 오늘 LIVE가 없는 홈 서버 cache를 30초로 줄이고 LIVE는 8초, 오늘이 아닌 날짜는 5분으로 분리해 현재 경기 전환이 오래된 홈 응답에 가려지는 시간을 줄임
- 시즌 전체 일정을 월 순서대로 유지하면서 한 번에 최대 3개월만 불러오고 기존 월별 cache를 재사용해 네트워크와 서버 부담을 제한
- Android와 웹에 표시되던 iOS 모양의 하단 막대를 제거하고 플랫폼 안전 영역만 사용하도록 정리
- 기본 화면의 설명·보조 라벨도 읽을 수 있는 대비를 유지하고, 경기 상세 뒤로가기에 명확한 접근성 이름을 제공하며, 240% 글자 크기의 필터·탭·경기 카드가 내용에 맞춰 재배치되도록 확대
- 데이터 브리핑을 켜 둔 채 KST 자정을 지나도 새 날짜의 경기·순위·기록 흐름으로 자동 전환하도록 개선
- 240% 글자 크기에서 기록실·전체 리더보드·알림 요약을 세로로 재배치하고 문자중계 필터의 터치 영역과 선택 상태 안내를 보강. 자동 생성 뒤로가기 이름도 한국어 `뒤로`로 통일
- 고대비 다크 화면의 선택 색이 모든 배경에서 4.5:1 이상 대비를 유지하도록 조정하고, 웹의 Cupertino 뒤로가기 아이콘 글꼴을 정식 포함
- 오래 사용하지 않은 push 등록을 device·push-to-start는 90일, Live Activity는 2일 뒤 정리하고 신규 owner 폭주를 60초당 120건으로 제한하되 기존 설치의 갱신·정상 token 교체는 계속 허용

### Fixed

- 라인업 timeout을 끝없는 자동 갱신처럼 표시하던 문제를 고쳐 오류 설명과 `다시 시도`를 제공하고, 큰 글자에서도 상태 카드가 잘리지 않도록 보정
- API 진단의 push 상태 조회 실패가 사라진 카드처럼 보이던 문제를 고쳐 loading·오류·재시도를 표시하고, 알림 읽음 저장 실패가 알림 상세 이동을 막지 않도록 보정
- 투수 정보만 확인된 박스스코어에서 팀 타율을 `0.000`으로 표시하던 문제를 `확인 불가`로 바꾸고, 현재/LIVE 경기에는 다른 경기나 과거 기록을 공식 박스스코어로 빌려오지 않도록 보정
- 과거 시즌 순위가 요청한 시즌과 다른 최신 표를 반복 표시할 수 있던 경로를 막고, KBO 원천 시즌과 기준일이 정확히 일치하는 순위만 사용하도록 보강
- 라인업 화면 조회가 알림 발송을 만들 수 있던 경로를 제거하고, Live Activity 갱신·종료는 인증된 운영 경로와 해당 설치의 등록 정보가 일치할 때만 처리하도록 보강
- 같은 sync에서 문자중계를 중복 조회하거나 내용이 바뀌지 않은 Live Activity를 반복 전송하던 경로를 줄이고, 일부 기기 전송 실패 시 실패한 대상만 다시 시도하도록 보정
- Live Activity 종료 요청이 실패한 뒤 같은 경기의 새 세션이 시작돼도 이전 세션 정보를 덮어쓰지 않고 정확한 등록 세대별로 재시도하며, 손상되거나 과도하게 쌓인 대기 항목을 정리하도록 보강
- Live Activity token 교체 요청이 다른 경기·Activity·설치의 기존 token을 삭제하지 못하도록 등록 owner가 정확히 일치할 때만 이전 token을 교체하도록 보강
- 일정 달력에서 월 데이터를 새로고친 뒤 매치업·구장별 화면으로 돌아가면 갱신 전 시즌 일정이 다시 보이던 문제를 보정
- iOS 재시작 뒤 native가 이전 Live Activity token을 비워 보내도 앱에 저장된 이전 token으로 안전하게 교체하고, 연속 token 교체가 서로의 등록 owner를 앞질러 저장하지 않도록 직렬화
- 공개 push 등록·기기 테스트·receipt가 설치 owner와 요청 크기 한계를 확인하고, registry 용량 초과나 손상 상태에서 기존 등록을 덮어쓰지 않도록 보강
- 늦게 도착한 push-to-start token 교체 요청이 현재 token을 되돌리지 못하도록 이전 token이 정확히 일치할 때만 교체하고, 이미 등록된 현재 token 재요청은 안전한 멱등 성공으로 처리
- 진행 중 공식 점수가 하향 정정될 때 한 번의 일시적 원천 회귀로 버리지 않도록 같은 정정값이 8초 뒤 다시 확인된 경우에만 반영하고, 잘못된 득점·역전 알림은 만들지 않도록 보강
- APNs가 `BadDeviceToken`, `DeviceTokenNotForTopic`, `Unregistered`로 영구 폐기한 Live Activity token을 registry와 전송 상태에서 제거해 5초마다 같은 실패를 반복하지 않도록 보정. 전송 중 같은 token 값이 새 세대로 다시 등록돼도 오래된 실패가 새 세션을 삭제하지 않으며, 일시 오류와 rate limit은 기존 재시도를 유지
- 과거 `SUSPENDED` 경기를 종료 스냅샷이나 30일 기기 cache로 고정하던 문제를 고쳐 공식 `FINAL`/`CANCELLED`만 재사용하고, 중단 경기는 재진입 때 원천 상태를 다시 확인
- 공개 `forceRefresh=true` 요청이 서버 cache를 무제한 우회하던 경로를 막고, 설정된 운영 비밀을 정확히 보낸 내부 동기화 요청만 실제 강제 갱신하도록 보강
- 여러 Live Activity token을 순차 발송할 때 늦은 구 점수가 새 점수 뒤에 도착할 수 있던 경로를 token별 revision·보내기 직전 fence·lease 갱신으로 차단
- iOS 시작 동기화가 최대 32개 이전 unregister 네트워크 재시도를 모두 기다리던 문제를 비차단 background drain으로 바꾸고, 저장할 수 없는 과대 historical 응답이 같은 key의 오래된 cache를 남기던 문제를 보정
- 새 backend가 먼저 배포돼도 `installationId`를 아직 보내지 않거나 Live Activity token/activity id가 부분적으로 비어 있는 기존 앱 요청이 422로 끊기지 않도록 200 안전 무동작으로 호환하고, 현재 앱의 완전한 정확 owner 요청만 발송·기록·삭제하도록 유지

## [0.1.23] - 2026-07-24

### Changed

- 모바일에서 순위를 `기록` 탭 소속으로 일관되게 표시하고, 태블릿·폴더블에서는 순위를 포함한 6개 왼쪽 탐색과 최대 720px 콘텐츠 폭을 사용하도록 개선
- 탭 이동과 상세 진입 시간을 짧게 조정해 점수·중계 확인 흐름을 빠르게 하고, 280/320px·240% 글자 크기에서는 경기 상단 점수판·탭·문자중계 점수판이 내용에 맞춰 재배치되도록 개선
- OS 고대비 light/dark 화면을 별도 팔레트로 제공하고, 공통 조작 영역을 44px 이상으로 맞추며 키보드·스크린리더의 label/hint/선택 상태를 보강
- 잠금 화면과 Dynamic Island에서 점수가 아직 확인되지 않은 `–`를 VoiceOver가 `점수 확인 중`으로 읽도록 보강
- 온보딩의 마이팀 저장이 알림 권한이나 네트워크 등록을 기다리지 않고 끝나는 즉시 홈으로 이동하도록 변경하고, OS 알림 권한은 사용자가 알림 CTA를 선택했을 때만 요청

### Fixed

- 원천 점수가 비어 있는 LIVE·종료·중단 경기를 실제 0:0처럼 표시하거나 득점·역전 알림으로 계산하던 문제를 고쳐 앱·위젯·Live Activity에서 `–`와 점수 확인 중 상태를 유지
- 서스펜디드 경기를 종료처럼 팔로우 해제하던 문제를 고쳐 중단 상태에서도 같은 경기 알림·Live Activity 세션을 보존
- 여러 알림이 동시에 도착하거나 읽는 순간 같은 이벤트가 재수신될 때 알림이 사라지거나 읽음 상태가 되돌아갈 수 있던 문제를 보정
- 일부 알림 대상 전송이 실패했을 때 성공한 대상에는 중복 발송하지 않고 실패한 대상만 다시 시도해 경기 이벤트가 영구 누락될 위험을 낮춤
- 여러 서버 작업이 동시에 실행돼도 야구 브리프와 Live Activity 시작·종료가 중복 발송되지 않도록 하고, 부분 실패한 브리프는 실패 대상만 다시 시도하도록 보정
- 늦게 도착한 과거 점수·이닝·타석·경기 상태가 최신 상태를 되돌리거나 득점·종료·타석 알림을 다시 만들던 문제를 보정하되, 공식 종료·취소 시점의 점수 정정은 최종 결과로 수용
- 마이팀 경기 카드의 전체 tap 안에 `중계 보기`·`알림 받기` 버튼이 중첩되어 포인터·키보드·스크린리더 동작이 겹칠 수 있던 문제를 보정

## [0.1.22] - 2026-07-23

### Changed

- 기록실 약어에 한글 의미를 함께 표시하고, 기존 `wRC+` 표기를 실제 계산 의미에 맞는 `OPS 상대지수`와 `앱 계산` 설명으로 교체
- 280px·240% 글자 크기에서 온보딩 혜택과 팀 카드, 박스스코어 선수·팀·요약 지표가 잘리지 않도록 유동 높이와 두 단 리플로우를 적용
- 일정 예매 시각에 KST와 공식·예상 출처를 표시하고, 캘린더 날짜를 전체 날짜·오늘·선택·경기 유무까지 읽도록 보강하며 320px 이하 보기 전환명을 한 줄로 축약
- 300px 이하 순위 헤더를 짧게 조정하고 경기 차·연속 기록의 의미를 화면에 설명

### Fixed

- 홈 갱신 실패 후 오래된 LIVE 점수에 최신처럼 보이던 문제를 고쳐 마지막 KST 갱신 시각, 지연 경고, 재시도를 표시
- 진행·종료·취소 맞대결이 `남은 경기`에 포함될 수 있던 문제를 고쳐 예정 경기만 집계
- 경기 팔로우 저장이 알림 권한·지원 환경과 무관하게 푸시 성공으로 표시되던 문제를 보정
- 경기·선수 콜드 딥링크 오류가 다시 시도하거나 안전한 루트로 돌아갈 수 없는 막다른 화면이 되던 문제를 보정
- 취소 경기가 공식 0:0처럼 보이거나, 이닝 기록이 없는데 전부 대시인 표를 표시하던 문제를 상태별 설명으로 교체

## [0.1.21] - 2026-07-23

### Changed

- 하단 `뉴스` 탭과 내부 화면을 실제 정보 성격에 맞는 `브리핑` / `데이터 브리핑`으로 바꾸고, KBO 데이터를 앱이 자동 정리한 화면이며 실제 뉴스 기사가 아님을 생성 시각과 함께 표시
- 홈런·승수 시즌 환산값을 `앱 계산`으로 구분하고, 순위 경기 차와 연속 승패에 맞는 표현만 사용하도록 데이터 브리핑 문구를 정리
- 기록실 첫 화면의 반복 리더 요약을 타자·투수 대표와 지표별 비교 중심으로 줄이고, 선수 상세 기록을 지표명·값 카드와 작은 화면 1열 형태로 재구성하면서 주요·시즌 기록의 같은 지표 반복을 제거
- 알림함에 최근 최대 50개 보관 범위와 현재 표시 수를 밝히고, 필터 선택 상태·터치 영역·목록/설정 독립 재시도를 보강
- 푸시 선호 상태를 `N개 선택됨`으로 표현하고, 실제 수신 가능성은 기기 권한·등록·서버 상태에 따라 달라질 수 있음을 안내
- 320px 온보딩과 홈 마이팀·순위 요약, 일정 달력 범례, 순위 팀명·연속 흐름을 더 읽기 쉬운 반응형 형태로 조정

### Fixed

- 원천 데이터가 없는 홈 LIVE 카드가 고정 B/S/O와 루상 상태를 실제 상황처럼 표시하던 문제를 제거
- 스코어 탭이 9회까지만 표시하고 연장 이닝을 누락하던 문제를 고쳐, 팀 열을 고정한 동적 이닝 표와 R/H/E/B 범례를 제공
- 박스스코어가 앱 휴리스틱 최고 타자를 공식 `결승타`처럼 표시하거나 원시 오류 문자열을 노출하던 문제를 보정
- 박스스코어 투수 파생값을 공식 기록처럼 보이게 하던 표현과 320px 활약 행에서 선수명이 지나치게 좁아지던 문제를 보정
- 라인업 비교 수치가 모두 없는데도 큰 빈 선발 비교표가 공개 타순보다 먼저 나오던 문제를 보정
- 데이터 브리핑 순위 제목에 `선두가 위태로운` 같은 접두사가 중복되거나 큰 경기 차에도 `턱밑`을 사용하던 문제를 보정
- 알림 목록 또는 설정 로드 실패를 정상 빈 상태나 무한 loading처럼 표시하던 문제를 보정
- 알림 목록을 불러오지 못한 상태에서 unread 수를 `정리됨`으로 확정하던 문제와 320px standalone 순위표의 고정 열 넘침을 보정

## [0.1.20] - 2026-07-23

### Changed

- 320px 화면과 큰 글자에서 온보딩 팀 선택을 한 열로 전환하고, 하단 탭·월 이동·건너뛰기 터치 영역과 비선택 텍스트 대비를 보강
- 키보드 Enter/Space 조작, 눈에 보이는 포커스, 스크린리더의 버튼·선택·비활성 상태를 공통 상호작용과 일정 선택 화면에 추가
- 마이팀 미선택 푸시 설정이 켜진 항목 수 대신 실제 대상 상태와 직접 팔로우 경기 알림의 별도 동작을 안내하도록 정리

### Fixed

- 신규 설치의 첫 홈을 업데이트 소식 팝업이 가리거나, 과거 seen-version 키가 없는 기존 설치를 신규 설치로 잘못 취급할 수 있던 문제를 보정
- 홈에서 마이팀을 선택한 뒤 홈이 아니라 설정으로 이동하던 복귀 경로를 보정
- 경기 상세 첫 조회 또는 후속 갱신 실패 시 기존 경기정보를 최신처럼 보이게 하던 문구를 제거하고 `갱신 지연`과 재시도를 표시
- 문자중계 바로 진입 시 첫 요약이 상단 탭 아래에 가리거나, 일정 마이팀 필터 뒤 구장별 보기에서 월 헤더가 화면 위로 밀리던 문제를 보정
- 구장별 일정 API 오류를 경기 없음으로 표시하거나 매치업의 비활성 팀을 키보드로 선택할 수 있던 문제를 보정

## [0.1.19] - 2026-07-22

### Fixed

- 홈 스코어보드에서 새로고침 표시가 실제 데이터 요청보다 먼저 끝나거나, 느린 요청을 다음 자동 갱신이 다시 시작해 최신 점수가 늦게 보일 수 있던 문제를 보정
- 홈 자동 갱신이 일시적인 API 오류 한 번 뒤 영구 중단되거나, 수동 새로고침과 기존 타이머가 겹칠 수 있던 문제를 보정
- 앱 복귀 동기화가 진행 중인 홈 스코어보드 요청을 다시 시작해 수동 새로고침 결과와 경쟁할 수 있던 문제를 보정
- 문자중계 자동 갱신 중 사용자가 당긴 새로고침이 무시되거나 첫 로딩을 5초 타이머가 다시 시작할 수 있던 문제를 보정
- 수동 스코어보드·문자중계 새로고침이 짧은 서버 cache나 과거 앱 cache만 다시 읽지 않고 최신 원천 조회를 요청하도록 보강
- 수동 강제 조회와 먼저 시작한 일반 조회가 겹쳐도 늦게 끝난 구 응답이 최신 서버 cache를 다시 덮지 않도록 보강
- 동시에 들어온 문자중계 요청이 하나의 KBO 로그인 세션 쿠키를 서로 초기화하거나, 내용 없는 중계 응답을 정상으로 표시할 수 있던 문제를 보정
...
- 홈/일정에서 경기 상세로 들어갈 때 첫 탭 데이터와 선수사진 준비를 기다리지 않고, 최신 경기정보 확인 뒤 바로 상세 화면으로 이동하도록 체감 속도를 개선
- 홈, 경기 상세, 기록 화면의 KBO 선수사진이 같은 전용 캐시와 화면 크기별 이미지 캐시를 재사용하도록 개선
- 경기 시작, 득점, 라인업, 타석 같은 경기 푸시가 마이팀 또는 직접 `푸쉬 중계 받기`로 따라가는 경기 외에는 먼저 오지 않도록 리그 전체 경기 moment topic 경로를 제거
- 2026 시즌 순위 bootstrap snapshot을 최신 순위 기준으로 갱신
