...
- 온보딩, 홈 빈 상태, 일정, 기록실, 알림 설정, 부트 스플래시에 AI 생성 야구 비주얼 리소스를 적용해 앱의 스포츠 앱 톤을 강화
- 알림 설정에 `야구 브리프`를 추가하고, backend가 월요일 주간 체크/비경기일/기록실 확인 같은 다양한 야구 정보 push 메시지를 보낼 수 있도록 확장
