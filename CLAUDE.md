...
- 2026-09-07 리뉴얼의 기능/경쟁/구현/검증 범위: `docs/RENEWAL_2026-09-07.md` 및 연결된 기능 감사·시장 조사. 내 팀 경기 우선, 기록 가이드/동일 팀·시즌 선수 비교, 핵심 중계, 다음 예정일, 알림 프리셋은 `0.1.32`에 반영됐고 Android build compatibility fix는 `0.1.33` 대상이다. 실기기/테스터 배포 증거와 구분한다.
...
- identity와 완성도 검증을 통과한 종료 경기, 과거 일정·순위, 과거 시즌 기록실 응답은 시간 만료만으로 background 재검증하지 않고 local cache에서 재사용한다. 명시적 force, cache key/schema 변경, 사용자 데이터 초기화, 용량 eviction만 재조회 경계로 두며 summary-only relay·비공식/빈 boxscore·불완전 lineup·rank gap payload는 immutable history로 저장하지 않는다
...
- 공통 하단 탭은 모바일 기준 `홈 / 일정 / 기록 / 브리핑 / 설정` 5개 탭이며, `홈`을 첫 번째에 둔다. `/standings`와 `/records` 하위 route는 모바일 `기록` 목적지 소속이고, 700px 이상 rail에서는 순위가 독립 목적지로 노출된다
