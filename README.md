...
- `GET /api/push/config-status`: `X-Kbo-Push-Sync-Secret`이 일치할 때만 Firebase/APNs/registry/scheduler 설정 누락을 secret 값 노출 없이 점검하며, secret 미설정·불일치는 503/401로 거절
...
- `RELEASE_API_HEALTH_PERFORMANCE=true RELEASE_API_HEALTH_DATE=YYYY-MM-DD ./scripts/release-api-health-check.sh https://<release-api>/api`: DNS/TLS와 주요 API envelope을 확인하면서 endpoint별 응답 시간을 cold/warm 두 단계로 비교합니다. 기본값은 단일 gate이며, performance 모드는 read-only 순차 요청입니다.
