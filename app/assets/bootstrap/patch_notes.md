...
## 0.1.35+103 - 라이브 경기 상세를 더 빠르고 안정적으로

- 빨라졌어요: 라이브 경기 상세에서 게임 정보와 문자중계·박스스코어·라인업을 더 빠르게 준비해 탭을 바꿀 때 기다리는 시간을 줄였습니다.
- 고쳤어요: KBO 원천 응답이 늦거나 일부 경기가 실패해도 다른 경기의 라이브 상태 갱신이 함께 멈추지 않도록 보강했습니다.
- 작게 다듬었어요: 큰 경기 기록 응답을 처리하는 동안 화면이 불필요하게 바빠지지 않도록 안정성을 높였습니다.

## 0.1.34+102 - 화면 전체가 더 읽기 쉬워졌어요

- 새로워졌어요: 홈부터 설정까지 모든 화면의 디자인을 같은 기준으로 다시 맞췄고, 팀 표시를 실제 KBO 구단 엠블럼으로 보여줍니다.
- 새로워졌어요: 홈 상단의 알림 아이콘에 읽지 않은 알림 수를 바로 표시합니다.
- 고쳤어요: 아직 시작하지 않은 경기를 0:0으로 표시하지 않고 `팀 vs 팀`과 시작 시각으로 안내합니다.
- 고쳤어요: 홈의 최근 경기를 불러오지 못했을 때 순위가 아닌 일정 화면으로 바로 이동합니다.
- 고쳤어요: 알림함·API 진단·업데이트 소식을 직접 열었을 때도 뒤로가기가 알맞은 화면으로 돌아갑니다.
- 빨라졌어요: 새로고침을 연속으로 눌러도 같은 요청이 중복 실행되지 않고 진행 상태를 보여줍니다.
- 작게 다듬었어요: 진행 중 경기를 알리는 빨간색은 경기 상태에만 쓰고, 선택·필터·로딩 표시는 파란색으로 구분했습니다.
- 작게 다듬었어요: 뒤로가기와 화면 읽기(접근성)가 중복 안내 없이 한 번씩 동작하도록 정리했습니다.

## 0.1.33+101 - Android 배포 빌드 안정화

- 고쳤어요: Android release 빌드에서 일부 위젯 모듈의 컴파일 설정이 달라 빌드가 멈추던 문제를 수정했습니다.
- 작게 다듬었어요: 0.1.32의 홈·경기·기록 리뉴얼은 그대로 유지하면서 배포용 빌드 호환성을 맞췄습니다.

## 0.1.32+100 - 내 팀 경기부터 기록까지 이어서 보기

- 새로워졌어요: 홈에서 내 팀의 오늘 경기 상태를 먼저 확인하고, 경기 결과와 팀 기록으로 바로 이어갈 수 있습니다.
- 새로워졌어요: 기록실에서 타율·OPS·ERA·WHIP·홈런·승·세이브·탈삼진·OPS 상대지수의 뜻과 읽는 법을 확인할 수 있습니다.
- 새로워졌어요: 같은 팀·같은 시즌 선수 두 명의 주요 비율과 제공되는 누적 기록을 나란히 비교할 수 있습니다.
- 새로워졌어요: 문자중계에서 득점·홈런·교체·경기 종료 같은 핵심 장면만 모아볼 수 있습니다.
- 새로워졌어요: 알림을 결과 중심·주요 순간·직접 설정 중에서 고를 수 있습니다.
- 고쳤어요: 취소된 경기나 점수가 아직 확인되지 않은 경기를 다음 경기 또는 0점으로 잘못 안내하지 않습니다.
- 고쳤어요: 과거 시즌 팀 기록으로 들어가도 선택한 시즌이 유지됩니다.
- 작게 다듬었어요: 경기 없는 날의 다음 일정, 예매 시각, 큰 글자 화면, 좁은 화면의 긴 선수명이 더 분명하게 보입니다.

## 0.1.31+99 - 박스스코어와 라인업도 놓치지 않게

- 고쳤어요: 박스스코어나 공개된 라인업을 열어 둔 상태에서 잠시 불러오기에 실패해도 자동으로 다시 확인합니다.
- 빨라졌어요: 정상적으로 확인한 박스스코어와 라인업을 잠시 저장해 네트워크가 불안정할 때도 마지막 내용을 바로 보여줍니다.
- 작게 다듬었어요: 저장된 기록을 보여줄 때 최신 갱신이 지연된 상태임을 화면에 안내합니다.

## 0.1.30+98 - 문자중계를 놓치지 않게

- 고쳤어요: 문자중계 탭을 열어 둔 채 경기 정보 응답이 늦어져도 중계 로딩이 멈추지 않고 자동으로 다시 확인합니다.
- 빨라졌어요: 한 번 확인한 문자중계는 잠시 저장해 네트워크가 불안정할 때도 마지막 내용을 바로 확인할 수 있습니다.
- 작게 다듬었어요: 저장된 중계를 보여줄 때 최신 갱신이 지연된 상태임을 화면에 안내합니다.

## 0.1.29+97 - 문자중계를 바로 확인하기

- 빨라졌어요: 경기 상세에 들어가면 문자중계 탭이 새로고침을 누르지 않아도 바로 데이터를 확인하기 시작합니다.
- 고쳤어요: 경기 전 정보가 실제 LIVE 경기로 바뀌거나 문자중계 탭으로 이동한 뒤 로딩이 계속 남아 있던 문제를 보완했습니다.
- 작게 다듬었어요: 이미 불러오는 중인 최신 중계 요청은 중복으로 다시 시작하지 않고 화면에 이어서 표시합니다.

## 0.1.28+96 - 과거 경기와 기록을 더 빠르게 다시 보기

- 빨라졌어요: 한 번 정상적으로 확인한 과거 경기와 과거 시즌 기록은 다음에 열 때 기기에 저장된 내용을 바로 보여줍니다.
- 고쳤어요: 종료 경기의 득점 요약이나 비어 있는 기록처럼 아직 완성되지 않은 정보는 오래된 기록으로 고정하지 않습니다.
- 작게 다듬었어요: 현재 경기와 현재 시즌은 기존처럼 최신 정보를 확인하고, 완료된 과거 데이터만 재사용하도록 구분했습니다.

## 0.1.27+95 - 경기 상세 로딩과 기록 표시 보강

- 빨라졌어요: 일정에서 경기 상세로 들어갈 때 일정에 보이던 팀과 경기 상태를 먼저 보여주어 긴 빈 로딩을 줄였습니다.
- 빨라졌어요: 라인업과 박스스코어가 선수 사진이나 부가 정보를 기다리지 않고 확인 가능한 기록부터 보여줍니다.
- 고쳤어요: 종료 경기 문자중계가 늦게 도착해도 득점 요약을 먼저 보고, 상세 중계는 준비되는 대로 이어서 확인할 수 있습니다.
- 작게 다듬었어요: 경기 전 라인업이 아직 공개되지 않은 경우 불필요한 로딩 대신 공개 전 상태를 명확히 보여줍니다.

## 0.1.26+94 - 요청 마감과 경기 정보 신뢰성 보강

- 빨라졌어요: 홈·일정·경기 상세·기록 화면이 오래 응답하지 않는 경우에도 무한 로딩 대신 제한된 시간 안에 상태와 다시 시도를 보여줍니다.
- 고쳤어요: 잠시 늦게 도착한 서버 응답 때문에 확인되지 않은 점수나 최근 결과가 실제 기록처럼 보이지 않도록 보강했습니다.
- 새로워졌어요: 모바일 기록 영역에서 `순위표`와 `선수 기록`을 한 번의 탭으로 오갈 수 있습니다.
- 빨라졌어요: 라이브 경기판 갱신이 알림이나 백그라운드 전송이 늦어도 별도 주기로 계속 확인됩니다.
- 작게 다듬었어요: 좁은 화면과 큰 글자에서 홈·일정·설정·경기 상세의 긴 내용이 잘리지 않도록 세로로 자연스럽게 배치했습니다.
- 고쳤어요: 같은 데이터를 여러 화면에서 동시에 불러올 때 불필요한 중복 조회와 끝없이 이어지는 대기를 줄였습니다.

## 0.1.25+93 - 현재 경기와 큰 화면에서 더 안전한 흐름

- 새로워졌어요: 웹에서 경기·리더보드·선수 상세를 열면 주소와 뒤로가기가 현재 화면을 따라가 새로고침이나 공유 뒤에도 같은 곳으로 돌아옵니다.
- 고쳤어요: 공식 점수가 아직 확인되지 않은 진행·중단 경기는 실제 0:0처럼 보이지 않고 점수 확인 중 상태로 유지됩니다.
- 고쳤어요: 홈의 최근 결과가 잠시 비어도 공식 점수가 확인된 종료 경기만 보조해 잘못된 `최근 결과 없음` 표시를 줄였습니다.
- 고쳤어요: 리더보드·선수 기록·일정 오류 화면에서 화면을 나가지 않고 `다시 시도`할 수 있습니다.
- 작게 다듬었어요: 경기 상세, 홈 오늘 경기, 순위, 기록실, 알림, 업데이트 소식이 280/320px 화면과 큰 글자에서도 잘리지 않도록 자동으로 재배치됩니다.
- 빨라졌어요: 경기 하이라이트 제목과 Live Activity 전송이 불필요하게 순차 대기하지 않고, 같은 상태를 반복 저장·전송하는 작업을 줄였습니다.
- 고쳤어요: 과거 경기·선수·시즌 데이터가 다른 요청의 정보로 섞이지 않도록 확인하고, 확인할 수 없는 데이터는 안전한 오류·재시도 상태로 보여줍니다.

## 0.1.24+92 - 화면 전환과 데이터 신뢰성 보강

- 빨라졌어요: 홈과 일정에서 경기 카드를 누르면 필요한 데이터 준비를 기다리지 않고 경기 상세를 먼저 열고, 최신 정보는 화면에서 이어서 갱신합니다.
- 빨라졌어요: 긴 문자중계도 현재 화면에 필요한 내용부터 표시해 많은 타석이 쌓인 경기에서 첫 화면과 스크롤이 가벼워졌습니다.
- 고쳤어요: 경기·순위·기록 화면이 한국시간 자정을 지나도 새 경기일과 현재 시즌으로 자연스럽게 전환되며, 직접 고른 과거 시즌은 그대로 유지합니다.
- 고쳤어요: 아직 확인되지 않은 팀 기록과 경기 점수를 실제 0이나 공식 기록처럼 보이지 않게 하고, 현재 경기에서 다른 경기 정보를 잘못 빌려오지 않습니다.
- 고쳤어요: 라인업·알림 설정·API 진단을 불러오지 못했을 때 빈 화면이나 끝없는 로딩 대신 상태와 다시 시도를 보여줍니다.
- 고쳤어요: 알림과 Live Activity가 동시에 갱신되거나 일부 전송이 실패해도 최신 상태가 뒤집히거나 성공한 알림이 중복되지 않도록 보강했습니다.
- 고쳤어요: 중단된 경기를 종료로 오인하지 않고, 경기 재개 전까지 팔로우와 Live Activity를 유지합니다.
- 작게 다듬었어요: 작은 화면과 큰 글자, 스크린리더에서 기록실·문자중계·알림·경기 상세의 선택 상태와 뒤로가기를 더 명확하게 읽고 조작할 수 있습니다.
- 작게 다듬었어요: Android와 웹의 하단 화면 여백을 운영체제에 맞게 정리하고, 고대비 화면의 글자와 선택 색을 더 읽기 쉽게 조정했습니다.

## 0.1.23+91 - 점수와 알림 신뢰성 보강

- 고쳤어요: 아직 확인되지 않은 점수를 0:0으로 보이지 않게 하고 앱·위젯·Live Activity에서 `점수 확인 중`으로 구분합니다.
- 고쳤어요: 중단된 경기를 종료로 처리하지 않고 알림과 Live Activity가 재개를 기다리도록 보정했습니다.
- 고쳤어요: 여러 알림이 동시에 도착하거나 일부 전송이 실패해도 읽음 상태와 성공한 알림이 사라지거나 중복되지 않도록 보강했습니다.
- 새로워졌어요: 태블릿과 폴더블에서는 순위를 포함한 넓은 화면용 탐색을 제공하고, 모바일 순위는 기록 탭 소속으로 일관되게 표시합니다.
- 작게 다듬었어요: 고대비 화면과 작은 화면·큰 글자의 경기 상세, 문자중계, 마이팀 카드 조작을 더 명확하게 다듬었습니다.
- 빨라졌어요: 마이팀 선택이 끝나면 알림 권한과 네트워크 등록을 기다리지 않고 바로 홈으로 이동합니다.

## 0.1.22+90 - 극단 화면과 복구 흐름 보강

- 고쳤어요: 홈 경기판 갱신이 늦을 때 마지막 확인 시각과 지연 안내를 표시하고 바로 다시 시도할 수 있습니다.
- 고쳤어요: 경기·선수 링크를 처음 열 때 오류가 나도 다시 시도하거나 안전한 화면으로 돌아갈 수 있습니다.
- 고쳤어요: 취소 경기와 기록 미제공 상태가 공식 0:0 또는 비어 있는 이닝 표처럼 보이지 않도록 정리했습니다.
- 작게 다듬었어요: 기록 약어에 한글 뜻을 더하고 앱 계산 지표를 공식 기록과 명확히 구분했습니다.
- 작게 다듬었어요: 아주 작은 화면과 큰 글자에서도 온보딩, 박스스코어, 일정, 순위 정보를 더 읽기 쉽게 배치했습니다.
